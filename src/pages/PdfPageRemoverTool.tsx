...
function parsePages(input: string, total: number): Set<number> {
...
export default function PdfPageRemoverTool() {
  ...
  const remove = async () => {
    ...
    try {
      ...
      const toRemove = parsePages(pages, totalPages)
      const keep = doc.getPageIndices().filter(i => !toRemove.has(i))
      if (keep.length === 0) { setMsg('不能删除所有页面'); setLoading(false); return }
      const newDoc = await PDFDocument.create()
      const copied = await newDoc.copyPages(doc, keep)
      copied.forEach(p => newDoc.addPage(p))
      const bytes = await newDoc.save(); download(bytes, `removed_pages.pdf`)
      setMsg(`已删除 ${toRemove.size} 页，保留 ${keep.length} 页`); setOk(true)
  ...
  return (
    <div className="max-w-xl mx-auto animate-fadeInUp">
      <ToolHeader name="PDF" accent="删除页面" accentColor="text-neon-orange" desc="按页码范围删除 PDF 页面，保留其余" />
      <Card>
        ...
        <div className="space-y-1"><Label>要删除的页码</Label><Input value={pages} onChange={e => setPages(e.target.value)} placeholder="1, 3-5" /></div>
        <Button onClick={remove} disabled={loading || !file || !pages.trim()} className="w-full">{loading ? <Loader2 size={15} className="animate-spin" /> : '删除并导出'}</Button>
